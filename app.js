...
// ===== Injecte les éléments DOM manquants (modal, toasts) =====
function ensureDOM(){
  ...
  if(!document.getElementById('detailModal')){
    ...
    on('modalClose','click',()=>{document.getElementById('detailModal').classList.add('hidden');});
    const md=document.getElementById('detailModal');
    if(md)md.addEventListener('click',(e)=>{if(e.target===md)md.classList.add('hidden');});
  }
}
...
// ===== MOTEUR DE CORRÉLATION AUTONOME =====
function FI(engine) {
...
function renderSheet(en,r){
  return '<div style="padding:10px;border-left:3px solid var(--accent);margin:6px 0 10px;background:var(--bg-primary);border-radius:8px">'+
   '<p style="font-size:12px;color:var(--text-secondary);margin-bottom:8px">'+en.year+' • '+en.fuelType+' • '+en.power+' • '+en.engineCode+'</p>'+
   '<div class="dtc-list" style="margin-bottom:8px">'+r.dtc.map(cd=>'<span class="dtc-chip" data-code="'+cd+'">'+cd+'</span>').join('')+'</div>'+
   '<h4 style="color:var(--danger);margin:8px 0 6px">🔴 Spécifique modèle ('+r.specific.length+')</h4>'+(r.specific.length?r.specific.map(card).join(''):'<p style="font-size:11px;color:var(--text-muted)">Aucun.</p>')+
   '<h4 style="color:var(--warning);margin:8px 0 6px">🟠 Famille moteur ('+r.fam.length+')</h4>'+r.fam.map(card).join('')+
   '<h4 style="color:var(--text-secondary);margin:8px 0 6px">🔵 Généraux ('+r.com.length+')</h4>'+r.com.map(card).join('')+'</div>';}

async function runSelfTest(){const out=[];const ok=(c,m)=>out.push((c?'✅ ':'❌ ')+m);
  try{ok((await VEHICLES_DB.getBrands('FRANCE')).includes('Peugeot'),'1 France Peugeot');
    ...
    ok(r.fam.some(i=>/8 mm/.test(i.title)),'5 chaîne 8 mm');ok(r.dtc.includes('P20EE'),'6 P20EE');ok(r.fam.length>0,'7 famille OK');
    const p={engineCode:'1.2 PureTech 130',fuelType:'Essence',power:'130ch'};
    ok((await buildVehicleIssues('Peugeot','3008',2020,p)).fam.some(i=>/Courroie/.test(i.title)),'8 courroie');
    ok(!!(await DTC_DB.getByCode('P20EE')),'9 base DTC');}catch(e){ok(false,'Err '+e.message);}
  const n=out.filter(l=>l.indexOf('✅')===0).length;console.log('─── AUTOCONTRÔLE ───');out.forEach(l=>console.log(l));window.APP_SELFTEST=out;
...
function loadEngines(c,brand,model,engines){c.innerHTML='';
  ...
  Object.keys(by).sort().forEach(y=>{const l=document.createElement('div');l.style.cssText='font-size:12px;color:var(--text-muted);margin:8px 0 4px;font-weight:600';l.textContent='📅 '+y;c.appendChild(l);
    by[y].forEach(en=>{const it=document.createElement('div');it.className='engine-item';
      ...
      it.addEventListener('click',async()=>{ // fiche INLINE sous la motorisation
        ...
        sh.innerHTML=renderSheet(en,r);bindChips(sh);
      });
      c.appendChild(it);});});}

async function showModelIssues(brand,model){const modal=document.getElementById('detailModal'),body=document.getElementById('modalBody');if(!modal||!body)return;
  ...
  body.innerHTML='<h2>'+brand+' '+model+' — toutes motorisations</h2>'+(spec.length?spec.map(card).join(''):'<p style="color:var(--text-muted)">Ouvrez une motorisation pour la corrélation complète.</p>');
  bindChips(body);}
