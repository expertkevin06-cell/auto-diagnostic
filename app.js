...
// CSS injecté : accordéons sans coupe + fond 80% + fiche inline
function injectUICSS(){ if(document.getElementById('uicss'))return;
  const st=document.createElement('style');st.id='uicss';
  st.textContent='.modal{position:fixed;inset:0;background:rgba(0,0,0,.7);display:flex;align-items:center;justify-content:center;z-index:3000;padding:16px;}.modal.hidden{display:none!important;}.modal-content{background:#273449;border-radius:12px;padding:20px;max-width:600px;width:100%;max-height:88vh;overflow-y:auto;position:relative;border:1px solid #334155;}.modal-close{position:absolute;top:10px;right:10px;background:none;border:none;color:#94a3b8;font-size:20px;cursor:pointer;}.drawer.open .drawer-content{max-height:none!important;overflow:visible!important;}.sub-drawer.open .sub-drawer-content{max-height:none!important;overflow:visible!important;}.fault-sheet{margin:6px 0 12px;}body.custom-bg::before{background:rgba(15,23,42,.2)!important;}';
...
function renderSheet(en,r){const all=r.specific.concat(r.fam,r.com);
  return {html:'<div style="padding:10px;border-left:3px solid var(--accent);margin:6px 0 10px;background:var(--bg-primary);border-radius:8px">'+
   ...
   '<h4 style="color:var(--danger);margin:8px 0 6px">🔴 Spécifique modèle ('+r.specific.length+')</h4>'+(r.specific.length?r.specific.map((i,k)=>card(i,k)).join(''):'<p style="font-size:11px;color:var(--text-muted)">Aucun.</p>')+
...
// Ouvre la fiche INLINE sous la motorisation + débloque les conteneurs parents
async function toggleInlineSheet(it, brand, model, en) {
  ...
  const sh = document.createElement('div'); sh.className = 'fault-sheet';
  sh.innerHTML = '<div class="loading">Chargement...</div>';
  it.after(sh);
  let p = sh.parentElement;
  while (p && p !== document.body) {
    if (p.classList && (p.classList.contains('drawer-content') || p.classList.contains('sub-drawer-content'))) { p.style.maxHeight = 'none'; p.style.overflow = 'visible'; }
    p = p.parentElement;
  }
...
function loadEngines(c,brand,model,engines){c.innerHTML='';
  ...
  Object.keys(by).sort().forEach(y=>{const l=document.createElement('div');l.style.cssText='font-size:12px;color:var(--text-muted);margin:8px 0 4px;font-weight:600';l.textContent='📅 '+y;c.appendChild(l);
    by[y].forEach(en=>{const it=document.createElement('div');it.className='engine-item';
      ...
      it.addEventListener('click',()=>toggleInlineSheet(it,brand,model,en));
      c.appendChild(it);});});}
